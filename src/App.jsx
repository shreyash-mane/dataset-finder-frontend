...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", width: "100%", maxWidth: "100%", overflowX: "hidden", background: "#050b1a", color: "#e0e8ff", fontFamily: "'Syne',sans-serif" }}>
      ...
      <style>{`* { box-sizing: border-box; } body { margin:0; padding:0; overflow-x:hidden; } @media(max-width:600px){ .search-row{flex-direction:column!important} .factor-grid{grid-template-columns:1fr!important} .card-inner{flex-direction:column!important} .filter-row{flex-direction:column!important} }`}</style>
      ...
      {/* Nav */}
      <nav style={{ position:"sticky", top:0, zIndex:100, background:"rgba(5,11,26,0.95)", borderBottom:"1px solid #0d1b3e", padding:"10px 20px", display:"flex", alignItems:"center", justifyContent:"space-between", backdropFilter:"blur(10px)", flexWrap:"wrap", gap:8 }}>
        <span onClick={() => setPage("search")} style={{ fontWeight:800, fontSize:17, color:"#6fa3ef", cursor:"pointer", letterSpacing:1 }}>⬡ DatasetFinder</span>
        <div style={{ display:"flex", gap:8, alignItems:"center", flexWrap:"wrap" }}>
          {user ? (
            <>
              <button onClick={() => setPage("saved")} style={{ ...S.btn("ghost"), padding:"7px 12px", fontSize:12 }}>💾 My Library</button>
              <button onClick={() => setPage("search")} style={{ ...S.btn("ghost"), padding:"7px 12px", fontSize:12 }}>🔍 Search</button>
              <div style={{ display:"flex", alignItems:"center", gap:7, padding:"5px 10px", background:"rgba(30,58,138,0.2)", borderRadius:20, border:"1px solid #1e3a8a" }}>
                {user.avatar && <img src={user.avatar} style={{ width:22, height:22, borderRadius:"50%" }} alt="" />}
                <span style={{ fontSize:12, color:"#aabcd8" }}>{user.name}</span>
                <button onClick={logout} title="Logout" style={{ background:"none", border:"1px solid #1e3a8a40", borderRadius:6, color:"#6b7a9a", cursor:"pointer", fontSize:11, padding:"2px 7px" }}>Logout</button>
              </div>
            </>
          ) : (
            <>
              <button onClick={() => setPage("login")} style={{ ...S.btn("ghost"), padding:"7px 12px", fontSize:12 }}>Login</button>
              <button onClick={() => setPage("register")} style={{ ...S.btn("primary"), padding:"7px 12px", fontSize:12 }}>Sign Up</button>
            </>
          )}
        </div>
      </nav>
  ...
  return (
    <div style={{ maxWidth:900, width:"100%", margin:"0 auto", padding:"46px 20px 80px" }}>
      ...
      <div style={{ textAlign:"center", marginBottom:40 }}>
        ...
        <h1 style={{ fontSize:"clamp(26px,5vw,50px)", fontWeight:800, lineHeight:1.05, margin:"0 0 12px", background:"linear-gradient(135deg,#e0e8ff 0%,#6fa3ef 50%,#3b82f6 100%)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>
          ...
          <div style={{ marginTop:12, background:"rgba(8,15,40,0.97)", border:"1px solid #1e3a8a", borderRadius:14, padding:"14px 16px", textAlign:"left", maxWidth:640, margin:"12px auto 0" }}>
            ...
            <div className="factor-grid" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:6 }}>
              {Object.entries(FACTOR_META).map(([k,f]) => (
                <div key={k} style={{ padding:"7px 9px", background:"#050b1a", border:"1px solid #0d1b3e", borderRadius:8 }}>
            ...
            <div style={{ marginTop:10, padding:"14px 16px", background:"rgba(8,15,40,0.95)", border:"1px solid #1e3a8a", borderRadius:12 }}>
              <div className="filter-row" style={{ display:"flex", gap:12, flexWrap:"wrap" }}>
                ...
                <div style={{ flex:1, minWidth:140 }}>
                  ...
                  <input type="range" min="0" max="10" step="0.5" value={filterMinScore} onChange={e => setFilterMinScore(Number(e.target.value))}
                    style={{ width:"100%", accentColor:"#3b82f6" }} />
                </div>
              </div>

              {/* Reset filters */}
              <button onClick={() => { setSortBy("reliability"); setFilterSource(""); setFilterLicense(""); setFilterMinScore(0); }}
                style={{ ...S.btn("ghost"), padding:"5px 12px", fontSize:11, marginTop:10 }}>
                ✕ Reset Filters
              </button>
            </div>
          )}
      ...
      {error && <div style={{ padding:14, background:"rgba(255,90,90,0.1)", border:"1px solid rgba(255,90,90,0.3)", borderRadius:12, color:"#ff8a8a", fontSize:13 }}>{error}</div>}
                ...
                return (
                  <div key={i} style={{ ...S.card, border:`1px solid ${i===0?"#1e3a8a":"#0d1b3e"}` }}>
                    <div style={{ padding:"6px 18px", background:i===0?"linear-gradient(90deg,rgba(30,58,138,0.5),transparent)":"rgba(8,14,34,0.5)", display:"flex", alignItems:"center", gap:10, borderBottom:"1px solid #0d1b3e" }}>
                      ...
                      <span style={{ fontSize:10, fontWeight:700, color:rankColors[Math.min(i,2)], fontFamily:"'Space Mono',monospace", letterSpacing:2 }}>{rankLabels[Math.min(i,2)]||"RESULT"}</span>
                      {ds.year && <span style={{ marginLeft:"auto", fontSize:10, color:"#6b7a9a", fontFamily:"'Space Mono',monospace" }}>📅 {ds.year}</span>}
                    </div>
                    ...
                    <div style={{ padding:"14px 18px" }}>
                      <div className="card-inner" style={{ display:"flex", justifyContent:"space-between", gap:14, flexWrap:"wrap" }}>
                        <div style={{ flex:1, minWidth:180 }}>
                          <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:5, flexWrap:"wrap" }}>
                            <h2 style={{ margin:0, fontSize:16, fontWeight:700, color:"#e0e8ff" }}>{ds.name}</h2>
                            <span style={{ padding:"2px 7px", background:"rgba(59,130,246,0.12)", border:"1px solid rgba(59,130,246,0.25)", borderRadius:5, fontSize:10, color:"#6fa3ef", fontFamily:"monospace" }}>{ds.source}</span>
                          </div>
                          <p style={{ margin:"0 0 8px", color:"#8899bb", fontSize:13, lineHeight:1.6 }}>{ds.description}</p>
                          <div style={{ display:"flex", gap:5, flexWrap:"wrap" }}>
                            {(ds.tags||[]).map((tag,t) => (
                              <span key={t} style={{ padding:"2px 7px", background:"#0d1b3e", border:"1px solid #1e3a7a", borderRadius:20, fontSize:10, color:"#6fa3ef", fontFamily:"monospace" }}>{tag}</span>
                            ))}
                          </div>
                        </div>
                        <div style={{ minWidth:145 }}>
                          <div style={{ padding:"10px 12px", background:"#050b1a", border:`1px solid ${rColor}44`, borderRadius:12, textAlign:"center", marginBottom:7 }}>
                            <div style={{ fontSize:9, color:"#6b7a9a", fontFamily:"'Space Mono',monospace", letterSpacing:1, marginBottom:2 }}>RELIABILITY</div>
                            <div style={{ fontSize:30, fontWeight:900, color:rColor, fontFamily:"monospace", lineHeight:1 }}>{sc}<span style={{ fontSize:12, color:rColor+"88" }}>/10</span></div>
                            <div style={{ fontSize:11, fontWeight:700, color:rColor, marginTop:2 }}>{ds.reliabilityLabel}</div>
                          </div>
                          <ScoreBar score={sc} />
                        </div>
                      </div>
                      ...
                      <div style={{ display:"flex", gap:6, marginTop:10, flexWrap:"wrap" }}>
                        {[["SIZE",ds.size],["FORMAT",ds.format],["CITATIONS",(ds.citedInPapers||0).toLocaleString()],["LICENSE",ds.license],["UPDATED",ds.lastUpdated]].map(([l,v],idx) => (
                          <div key={idx} style={{ padding:"5px 9px", background:"#0a1228", border:"1px solid #0d1b3e", borderRadius:7 }}>
                      ...
                      <div style={{ borderTop:"1px solid #0d1b3e", padding:"16px 18px", background:"rgba(4,9,22,0.7)" }}>
                        ...
                        <div style={{ display:"flex", flexDirection:"column", gap:5, marginBottom:14 }}>
                          {sortedPapers.map((p,pi) => (
                            <div key={pi} style={{ padding:"8px 10px", background:"#080f2a", border:"1px solid #0d1b3e", borderRadius:7 }}>
                        ...
                        <a href={ds.url} target="_blank" rel="noopener noreferrer"
                          style={{ display:"block", padding:"10px", background:"linear-gradient(135deg,#1d4ed8,#3b82f6)", borderRadius:9, color:"#fff", textDecoration:"none", textAlign:"center", fontWeight:700, fontSize:13 }}>
                          View Dataset Source →
                        </a>
                      </div>
                    )}
      ...
      {!loading && !results && !error && (
        <div style={{ textAlign:"center" }}>
          <div style={{ display:"flex", justifyContent:"center", gap:7, flexWrap:"wrap", marginBottom:16 }}>
            {["image classification","mental health AND depression","climate data","NLP OR sentiment","medical imaging","fraud detection"].map(s => (
              <button key={s} onClick={() => setQuery(s)} style={{ padding:"6px 13px", background:"rgba(10,20,50,0.8)", border:"1px solid #1e3a8a", borderRadius:20, color:"#6fa3ef", fontSize:11, cursor:"pointer", fontFamily:"'Syne',sans-serif" }}>{s}</button>
            ))}
          </div>
          <p style={{ color:"#2a3a5a", fontSize:11, fontFamily:"'Space Mono',monospace" }}>← TRY A SAMPLE SEARCH · SUPPORTS AND/OR QUERIES</p>
        </div>
      )}
    </div>
  );
...
function SavedPage({ user, setPage }) {
  ...
  const downloadPdf = (id) => window.open(`${API_BASE}/api/saved/${id}/pdf`, "_blank");
      ...
      <div style={{ display:"grid", gridTemplateColumns:"200px 1fr", gap:16 }}>
        ...
        <div>
          <div style={{ ...S.card, padding:14, marginBottom:10 }}>
            ...
            <div onClick={() => setActiveCol(null)} style={{ padding:"7px 9px", borderRadius:7, cursor:"pointer", background:!activeCol?"rgba(59,130,246,0.15)":"transparent", border:!activeCol?"1px solid #3b82f640":"1px solid transparent", marginBottom:3, fontSize:12, color:!activeCol?"#6fa3ef":"#8899bb" }}>
              📋 All ({datasets.length})
            </div>
            {collections.map(col => (
              <div key={col._id} onClick={() => setActiveCol(col._id)} style={{ padding:"7px 9px", borderRadius:7, cursor:"pointer", background:activeCol===col._id?"rgba(59,130,246,0.15)":"transparent", border:activeCol===col._id?"1px solid #3b82f640":"1px solid transparent", marginBottom:3, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                  ...
                  <div key={ds._id} style={S.card}>
                    <div style={{ padding:"12px 16px" }}>
                      ...
                      <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                        ...
                        <button onClick={() => downloadPdf(ds._id)} style={{ ...S.btn("ghost"), padding:"4px 10px", fontSize:10 }}>📄 PDF</button>
                        <button onClick={() => deleteDs(ds._id)} style={{ ...S.btn("danger"), padding:"4px 10px", fontSize:10 }}>🗑 Remove</button>
                      </div>
                      {editingNote===ds._id && (
                        <div style={{ marginTop:10 }}>
                          <textarea value={noteText} onChange={e => setNoteText(e.target.value)} placeholder="Add your notes..."
...
function Loading() {
  return (
    <div style={{ display:"flex", justifyContent:"center", alignItems:"center", height:"60vh" }}>
      <div style={{ width:38, height:38, border:"3px solid #1e3a8a", borderTopColor:"#3b82f6", borderRadius:"50%", animation:"spin 0.8s linear infinite" }} />
      <style>{`@keyframes spin { to { transform:rotate(360deg); } }`}</style>
    </div>
  );
}
